...
First, "directed" links are exactly like normal links except that there is an inherent direction or order embedded within them. For example, in a road map, a one-way street might be represented as a directed link because you can travel from one end to the other, but not the other way. For more information, see [here](www.example.com). 

Second, link properties are just variables that are "owned" by each link in the same way that a turtle or a patch can "own" a variable. Each link has its own distinct copy of a link-owned variable. For more, see [here](www.example.com). 

In this example, links are used to represent air travel routes between three cities, Chicago, New York, and Boston, across their five major airports, Chicago Midway (MDW), Chicago O'Hare (ORD), New York John F. Kennedy (JFK), New York LaGuardia (LGA), and Boston Logan (BOS). By changing the "airport-choice" picker, you can see which airports are connected to which, in other words, what airports you can fly to directly from which.
@#$#@#$#@
default
true
0
